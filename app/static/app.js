...
function setupEventListeners() {
    ...
    const shareBtn = document.getElementById("share-now-btn");
    ...
    // Share Analysis via here.now
    shareBtn.addEventListener("click", async () => {
        const modal = document.getElementById("share-modal");
        const loadingContainer = document.getElementById("share-loading-container");
        ...
        // Find the last assistant message HTML to share
        const chatHistory = document.getElementById("chat-history");
        const assistantMsgs = chatHistory.querySelectorAll(".assistant-message");
        let lastAssistantHtml = "";
        
        if (assistantMsgs.length > 0) {
            const lastMsg = assistantMsgs[assistantMsgs.length - 1];
            const textEl = lastMsg.querySelector(".message-text-content");
            if (textEl) {
                lastAssistantHtml = textEl.innerHTML;
            }
        }
        
        if (!lastAssistantHtml) {
            lastAssistantHtml = "<p>Data analysis insights generated by Ask-Your-Data Genie.</p>";
        }

        try {
            const response = await fetch("/api/share/here-now", {
                method: "POST",
                headers: getHeaders(),
                body: JSON.stringify({
                    title: "Ask-Your-Data Analysis Insights",
                    textHtml: lastAssistantHtml,
                    chartJson: currentChartData
                })
            });
            
            const data = await response.json();
            if (!response.ok) {
                throw new Error(data.detail || "Failed to publish share");
            }
            
            document.getElementById("share-url-input").value = data.siteUrl;
            
            const claimWrapper = document.getElementById("share-claim-wrapper");
            const claimLink = document.getElementById("share-claim-link");
            
            if (data.claimUrl) {
                claimLink.href = data.claimUrl;
            ...
            alert("Error sharing analysis: " + err.message);
